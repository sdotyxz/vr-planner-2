...
## 3D浮动对话弹窗 - 使用 TextMesh 显示人质对话并向上飘动后消失

@onready var mesh_instance: MeshInstance3D = $TextMeshInstance
var text_mesh: TextMesh

## 动画参数
const FLOAT_HEIGHT: float = 0.8  ## 向上飘动高度
const DURATION: float = 1.2  ## 持续时间（比分数弹窗长）
const SCALE_START: float = 0.6  ## 起始缩放
const SCALE_PEAK: float = 1.1  ## 峰值缩放
...
func set_color(color: Color) -> void:
	...
	if material:
		...
		material.emission = Color(color.r * 0.5, color.g * 0.5, color.b * 0.5, 1.0)
...
func _animate() -> void:
	...
	# 缩放：先放大再恢复
	tween.tween_property(mesh_instance, "scale", Vector3.ONE * SCALE_PEAK, DURATION * 0.2).set_ease(Tween.EASE_OUT)
	tween.tween_property(mesh_instance, "scale", Vector3.ONE, DURATION * 0.8).set_delay(DURATION * 0.2).set_ease(Tween.EASE_IN_OUT)
	
	# 透明度：后半段淡出（通过材质实现）
	if material:
		material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
		tween.tween_property(material, "albedo_color:a", 0.0, DURATION * 0.4).set_delay(DURATION * 0.6).set_ease(Tween.EASE_IN)
